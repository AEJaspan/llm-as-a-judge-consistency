...
class LLMJudge:
    ...
    def _get_schema_and_description(self, confidence_type: str) -> Tuple[Type[BaseModel], str]:
        """Get schema and description for confidence type."""
        schema_map = {
            "float": (PredictionFloat, "a float between 0.0 and 1.0"),
            "categorical": (PredictionCategorical, "one of: very low, low, medium, high, very high"),
    ...
    def _create_chain(self, confidence_type: str, system_prompt: Optional[str] = None):
        ...
        schema, confidence_description = self._get_schema_and_description(confidence_type)
...
- Your certainty in the classification"""),
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if attempt == self.max_retries - 1:
                    logger.error(f"Final attempt failed for model {self.model_name}: {e}")
    ...
    async def classify_with_confidence(self, text: str, confidence_type: str, 
    ...
    def classify_with_confidence_sync(self, text: str, confidence_type: str,
                                     system_prompt: Optional[str] = None) -> Dict:
        ...
        try:
            ...
            try:
                ...
                logger.error(f"Error with model {self.model_name}, confidence_type {confidence_type}: {e}")
                return self._get_fallback_response(schema)
