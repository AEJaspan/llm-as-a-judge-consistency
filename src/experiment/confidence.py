...
from config.base_models import DatasetChoice, ExperimentConfig, CATEGORICAL_TO_FLOAT
...
class ConfidenceExperiment:
    ...
    async def _rate_limited_llm_call(
        self, judge: LLMJudge, text: str, conf_type: str, task_description: str
    ):
        """Make an LLM call with rate limiting"""
        async with self.semaphore:
            return await judge.classify_with_confidence(
                text, conf_type, task_description
            )
    ...
    def load_dataset(self):
        ...
        if (
            len(df) > self.config.sample_size
            and self.config.dataset_choice == DatasetChoice.SST2
        ):
        ...
        print(
            f"  Label distribution: {df[self.config.label_column].value_counts().to_dict()}"
        )
    ...
    async def run_experiment(self):
        ...
        confidence_types = self.config.confidence_types or [
            "float",
            "categorical",
            "integer",
        ]
        ...
        for i in range(0, len(all_tasks), batch_size):
            ...
            for task_result, metadata in zip(batch_results, batch_metadata):
                if isinstance(task_result, Exception):
                    ...
                    task_result = {"classification": None, "confidence": None}
    ...
    def _compare_models(self, df: pd.DataFrame):
        ...
        if len(consistency_df) > 0:
            ...
            if len(conf_type_groups) > 2:
                ...
                comparison_results["confidence_type_anova"] = {
                    "f_stat": f_stat,
                    "p_value": p_value,
                }
            ...
            if len(model_groups) > 2:
                ...
                comparison_results["model_anova"] = {
                    "f_stat": f_stat,
                    "p_value": p_value,
                }
    ...
    def create_visualizations(self, results_df: pd.DataFrame, analyses: Dict):
        ...
        fig.suptitle(
            f"LLM Confidence Experiment Results - {dataset_name.upper()} Dataset",
            fontsize=16,
            fontweight="bold",
        )
        ...
        results_df["confidence_bin"] = pd.cut(
            results_df["normalized_confidence"],
            bins=5,
            labels=["Low", "Med-Low", "Med", "Med-High", "High"],
        )
        acc_by_conf = (
            results_df.groupby(["confidence_type", "confidence_bin"])["correct"]
            .mean()
            .reset_index()
        )
        sns.lineplot(
        ...
        model_perf = (
            results_df.groupby(["model", "confidence_type"])
            .agg({"correct": "mean", "normalized_confidence": "mean"})
            .reset_index()
        )

        sns.scatterplot(
            data=model_perf,
            x="normalized_confidence",
            y="correct",
            hue="model",
        ...
        pivot_data = consistency_df.pivot(
            index="Model", columns="Confidence Type", values="Mean CV"
        )
