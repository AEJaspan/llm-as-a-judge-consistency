...
class PredictionFloat(BaseModel):
    classification: bool = Field(
        ..., description="the binary classification of the text"
    )
    confidence: float = Field(
        ..., ge=0.0, le=1.0, description="the confidence in the classification"
    )
...
class PredictionCategorical(BaseModel):
    ...
    confidence: Literal["very low", "low", "medium", "high", "very high"] = Field(
        ..., description="the confidence in the classification"
    )
...
class PredictionInteger(BaseModel):
    ...
    confidence: int = Field(
        ..., ge=0, le=5, description="the confidence in the classification"
    )
...
class ExperimentConfig:
    ...
    def __post_init__(self):
        ...
        if self.sample_size < ExperimentConstants.MIN_SAMPLE_SIZE:
            raise ValueError(
                f"Sample size must be at least {ExperimentConstants.MIN_SAMPLE_SIZE}"
            )
        if self.sample_size > ExperimentConstants.MAX_SAMPLE_SIZE:
            raise ValueError(
                f"Sample size cannot exceed {ExperimentConstants.MAX_SAMPLE_SIZE}"
            )
